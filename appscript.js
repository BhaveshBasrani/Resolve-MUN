...
function handleDashboardActions(ss, data) {
    ...
    try {
        ...
        switch (action) {
            ...
            case "ADMIN_APPROVE_DELEGATION":

                return adminApproveDelegation(ss, data);
...
function adminGetData(ss) {
    ...
    const data = {
        ...
        oc: getSheetDataAsObjects(ss.getSheetByName(SHEET_NAMES.OC)),

        delegations: getSheetDataAsObjects(ss.getSheetByName(SHEET_NAMES.DELEGATION)),

        eb: getSheetDataAsObjects(ss.getSheetByName(SHEET_NAMES.EB))
...
function adminApproveDelegation(ss, data) {
    ...
    const delSheet = ss.getSheetByName(SHEET_NAMES.DELEGATION);

    const delegationVal = delSheet.getRange(rowIndex, 1, 1, delSheet.getLastColumn()).getValues()[0];



    const instName = delegationVal[1];

    const adviserName = delegationVal[2];

    const adviserPhone = delegationVal[3];

    const adviserEmail = delegationVal[4];

    const delegateCount = parseInt(delegationVal[5], 10) || 0;
    ...
    if (data.allocations && Array.isArray(data.allocations)) {
        ...
        data.allocations.forEach(a => {
            ...
            allocationsMap[a.delegateIndex] = {
    ...
    const indivSheet = ss.getSheetByName(SHEET_NAMES.DELEGATE);

    if (indivSheet.getMaxColumns() < 35) {

        indivSheet.insertColumnsAfter(indivSheet.getMaxColumns(), 35 - indivSheet.getMaxColumns());
    ...
    for (let i = 1; i <= delegateCount; i++) {
        ...
        const colStart = 9 + (i - 1) * 5;

        const name = String(delegationVal[colStart] || "").trim();

        const phone = String(delegationVal[colStart + 1] || "").trim();

        const email = String(delegationVal[colStart + 2] || "").trim();

        const prefComm = String(delegationVal[colStart + 3] || "").trim();

        const prefCoun = String(delegationVal[colStart + 4] || "").trim();
        ...
        const alloc = allocationsMap[i] || {};

        const allocatedCommittee = alloc.committee || prefComm || "TBD";

        const allocatedCountry = alloc.country || prefCoun || "TBD";



        let existingRowIndex = -1;

        const lastIndivRow = indivSheet.getLastRow();

        if (lastIndivRow > 1) {

            const emails = indivSheet.getRange(2, 5, lastIndivRow - 1, 1).getValues().flat();

            for (let j = 0; j < emails.length; j++) {

                if (String(emails[j]).toLowerCase() === email.toLowerCase()) {

                    existingRowIndex = j + 2;
        ...
        if (existingRowIndex > -1) {
            ...
            const existingVals = indivSheet.getRange(existingRowIndex, 1, 1, 35).getValues()[0];

            delegateId = existingVals[22] || generateDelegateId(indivSheet);

            password = existingVals[31] || generatePassword();



            indivSheet.getRange(existingRowIndex, 2).setValue(name);

            indivSheet.getRange(existingRowIndex, 4).setValue(phone);

            indivSheet.getRange(existingRowIndex, 22).setValue("Approved");

            indivSheet.getRange(existingRowIndex, 23).setValue(delegateId);

            indivSheet.getRange(existingRowIndex, 24).setValue(allocatedCommittee);

            indivSheet.getRange(existingRowIndex, 25).setValue(allocatedCountry);

            indivSheet.getRange(existingRowIndex, 26).setValue("Verified");

            indivSheet.getRange(existingRowIndex, 31).setValue(`Delegation: ${instName}`);

            indivSheet.getRange(existingRowIndex, 32).setValue(password);

        } else {

            delegateId = generateDelegateId(indivSheet);
            ...
            const newRowIndex = indivSheet.getLastRow();

            indivSheet.getRange(newRowIndex, 32).setValue(password);

            indivSheet.getRange(newRowIndex, 33).setValue("Absent");

            indivSheet.getRange(newRowIndex, 34).setValue("Absent");

            indivSheet.getRange(newRowIndex, 35).setValue("Absent");

        }



        sendCredentialsEmail(email, name, delegateId, password, allocatedCommittee, allocatedCountry);

        approvedDelegates.push({ name: name, email: email, delegateId: delegateId });

    }



    const maxDelCols = delSheet.getMaxColumns();

    if (maxDelCols < 10) {

        delSheet.insertColumnsAfter(maxDelCols, 10 - maxDelCols);

    }

    if (delSheet.getRange(1, 10).getValue() !== "Status") {

        delSheet.getRange(1, 10).setValue("Status");

    }

    delSheet.getRange(rowIndex, 10).setValue("Approved");
    ...
    return ContentService.createTextOutput(JSON.stringify({
        ...
        message: "Delegation approved. " + approvedDelegates.length + " delegates registered and emailed.",

        delegates: approvedDelegates
